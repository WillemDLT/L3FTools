...
do
	...
	function updatePosition(button, position)
		if button.dataObject and button.dataObject.forceParentUI then
			return
		end

		if button.db and button.db.freePosition and button.db.freeX and button.db.freeY then
			if button.dataObject and button.dataObject.allowFreeMove and button:GetParent() ~= UIParent then
				button:SetParent(UIParent)
			end
			button:ClearAllPoints()
			button:SetPoint("CENTER", UIParent, "CENTER", button.db.freeX, button.db.freeY)
			return
		end
		...
		elseif button.dataObject and button.dataObject.allowFreeMove and button:GetParent() ~= Minimap then
...
do
	...
	function onDragStart(self)
		if self.dataObject and self.dataObject.allowFreeMove and IsShiftKeyDown() then
			self.isFreeMoving = true
		...
		if self.dataObject and self.dataObject.forceParentUI then
			return
		end

		if self.db then
			if self.db.freePosition then
				return
			end
		end
		self:LockHighlight()
...
local function createButton(name, object, db)
	...
	button.baseOverlayPointX = 0
	button.baseOverlayPointY = 0
	...
	icon:SetPoint("TOPLEFT", 5.5, -5)
	button.icon = icon
	button.baseSize = 31
	button.baseOverlaySize = 53
	button.baseBackgroundSize = 20
	button.baseBackgroundPointX = 7
	button.baseBackgroundPointY = -5
	button.baseIconSize = 20
	button.baseIconPointX = 5.5
	button.baseIconPointY = -5
